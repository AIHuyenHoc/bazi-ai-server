...
const hasAssistantBefore = (messages) =>
...
const analyzeNguHanh = (tuTru) => {
  ...
  for (const chi of branches) {
    (hiddenElements[chi] || []).forEach((h) => { if (canNguHanh[h]) count[canNguHanh[h]] += 0.3; });
  }
  if (Object.values(count).every((v) => v === 0)) throw new Error("Không tìm thấy ngũ hành hợp lệ");
  return count;
};
...
const elementMeta = {
  Mộc:{ vi:{ env:"không gian nhiều cây xanh/ánh sáng tự nhiên", jobs:"giáo dục, thiết kế, nội dung, cố vấn phát triển", colors:"xanh lá, gỗ nâu" }, en:{ env:"greenery & natural light", jobs:"education, design, content, advisory", colors:"green, wood-brown" } },
  Hỏa:{ vi:{ env:"môi trường sôi động, trình diễn, spotlight", jobs:"truyền thông, sân khấu, năng lượng", colors:"đỏ, cam" }, en:{ env:"dynamic/spotlight settings", jobs:"media, stage, energy", colors:"red, orange" } },
  Thổ:{ vi:{ env:"hệ thống ổn định, quản trị – vận hành", jobs:"quản trị, bất động sản, vận hành", colors:"vàng, nâu đất" }, en:{ env:"stable systems & operations", jobs:"management, real estate, operations", colors:"yellow, earth-brown" } },
  Kim:{ vi:{ env:"quy chuẩn, dữ liệu, công nghệ – tài chính", jobs:"tài chính, kỹ thuật, pháp chế", colors:"trắng, ánh kim" }, en:{ env:"structured, data/tech/finance", jobs:"finance, engineering, compliance", colors:"white, metallic" } },
  Thủy:{ vi:{ env:"nghiên cứu, dữ liệu, di chuyển – kết nối", jobs:"CNTT, logistics, nghiên cứu", colors:"xanh dương, đen" }, en:{ env:"research, data, mobility", jobs:"IT, logistics, research", colors:"blue, black" } },
};
...
const detectTopic = (text) => {
  ...
  const tests = [
    // MONEY: thêm "tài vận", "làm ăn", "lộc", "doanh thu", "dòng tiền"
    ["MONEY", /(tai loc|tai van|van tai|loc|lam an|thu nhap|tai chinh|money|finance|income|salary|cashflow|danh thu|dong tien|kiem tien|profit|roi|invest)/],
    ["CAREER", /(su nghiep|nghe nghiep|thang tien|cong viec|career|job|promotion|startup|business|sales|marketing|product|developer|dev)/],
    ["LOVE", /(tinh cam|tinh yeu|hon nhan|ket hon|yeu|romance|relationship|lover|marriage|nguoi yeu)/],
    ["FAMILY", /(gia dinh|gia dao|cha me|vo chong|con chau|family|parents|spouse|home harmony|inlaws)/],
    ...
    ["TIMING", /(khi nao may man|thoi diem may man|bao gio tot|tuong lai may man|luckiest|timing|may man nhat)/],
    ["INVEST", /(co nen dau tu|dau tu gi|investment|risk|stocks|crypto|vay von)/],
    ["COMM", /(giao tiep|thuyet phuc|trinh bay|thuyet trinh|communication|present)/],
...
const genHeartfeltOverview = (lang, tuTru, nguHanh, thapThan, thanSat, dungThan) => {
  ...
  const godsCount = {};
  Object.values(thapThan || {}).forEach((n) => { if (!n) return; godsCount[n] = (godsCount[n]||0)+1; });
  const topGods = Object.entries(godsCount).sort((a,b)=>b[1]-a[1]).slice(0,4).map(([n,c])=>`${n} (${c})`).join(" · ");

  const stars = Object.values(thanSat || {}).filter((v)=>v.value && v.value.length)
    .map((v)=>`${v[vi ? "vi" : "en"]}: ${v.value.join(", ")}`).join(" | ");
  ...
  if (vi) {
    return [
      ...
      joinBullets([
        `Duy trì thói quen nuôi dưỡng **${chosen}** mỗi ngày.`,
        `20–30 phút cho hoạt động giúp tâm trí an định/sáng tạo.`,
        `Kết nối Quý Nhân bằng sự chân thành & chia sẻ giá trị mạnh của bạn.`,
      ]),
    ].filter(Boolean).join("\n");
  }
  return [
    ...
    `Five Elements: Wood ${pct(perc["Mộc"])} · Fire ${pct(perc["Hỏa"])} · Earth ${pct(perc["Thổ"])} · Metal ${pct(perc["Kim"])} · Water ${pct(perc["Thủy"])}.`,
    `Overall: **${dominant}** strong, **${weak}** weaker → **add ${chosen}** to balance.`,
    `Best environments: ${meta.env}. Suitable careers: ${meta.jobs}.`,
    topGods ? `Prominent Ten Gods: ${topGods}.` : "",
    stars ? `Active stars: ${stars}.` : "",
    joinBullets([
      `Nurture **${chosen}** daily.`,
      `20–30 minutes for grounding/creative practice.`,
      `Grow network through honest contributions.`,
    ]),
  ].filter(Boolean).join("\n");
};

const genMenuQuick = (lang) => {
  const vi = lang === "vi";
  return vi
    ? "Bạn có thể hỏi tiếp về **tài vận, sự nghiệp, tình cảm, sức khoẻ, gia đạo, con cái, tài sản/bất động sản, học tập, thời điểm may mắn, đầu tư, màu sắc**."
    : "You can ask next about **money, career, love, health, family, children, property, study, lucky timing, investing, colors**.";
};

const genTopicAnswer = (topic, lang, ctx, hasPrev) => {
  const vi = lang === "vi";
  ...
  switch (topic) {
    case "MONEY": {
      ...
      return vi
        ? [
            "**Tài vận:** xu hướng tích lũy **ổn định** nhờ " +
              (ct ? "Chính Tài (kỷ luật/thu nhập chính)" : "kỷ luật chi tiêu") +
              "; cơ hội bứt phá đến từ " +
              (tt ? "Thiên Tài (thu nhập phụ/linh hoạt)" : "kỹ năng mới và định giá tốt hơn") +
              ".",
            joinBullets([
              "Thiết lập 3 quỹ: chi tiêu – dự phòng 6 tháng – đầu tư đều.",
              "Ưu tiên tài sản có giá trị thật & dòng tiền; hạn chế lướt sóng.",
              `Tạo dòng thu phù hợp **${chosen}** (vd: ${elementMeta[chosen].vi.jobs}).`,
            ]),
          ].join("\n")
        : [
            "**Wealth:** steady accumulation via " +
              (ct ? "Direct Wealth (discipline)" : "budgeting") +
              "; upside from " +
              (tt ? "Indirect Wealth (side income/opportunistic)" : "upskilling and pricing your value") +
              ".",
            joinBullets([
              "Set 3 buckets: spending – 6-month buffer – consistent investing.",
              "Prefer intrinsic-value, cash-flowing assets; avoid hype trades.",
              `Build income streams aligned with **${chosen}** (e.g., ${elementMeta[chosen].en.jobs}).`,
            ]),
          ].join("\n");
    }
    // … (giữ nguyên các case còn lại như bản trước) …
    default:
      // Không lặp lại tổng quan ở các lượt sau
      return hasPrev ? genMenuQuick(lang) : genHeartfeltOverview(lang, tuTru, nguHanh, thapThan, thanSat, dungThan);
  }
};
...
app.post("/api/luan-giai-bazi", async (req, res) => {
  ...
  try {
    ...
    const firstLike = topic === "HEARTFELT" || (!hasAssistantBefore(messages) && topic === "GENERAL");
    let base = genTopicAnswer(topic, language, { tuTru, nguHanh, thapThan, thanSat, dungThan: dungThanHanh }, hasAssistantBefore(messages));
    if (firstLike) base = genHeartfeltOverview(language, tuTru, nguHanh, thapThan, thanSat, dungThanHanh);
    ...
    if (!keyOk) {
      cache.set(cacheKey, base);
      return res.json({ answer: base, warning: language === "vi" ? "OpenAI không khả dụng" : "OpenAI unavailable", took_ms: Date.now() - start });
    }

    const prompt =
      (language === "vi"
        ? "Hãy viết lại đoạn dưới đây theo văn phong gần gũi, rõ ràng; giữ nguyên thuật ngữ Bát Tự; không bịa mốc thời gian; tránh lặp ý.\n\n"
        : "Rewrite in a warm, clear tone; keep Bazi terms; no invented dates; avoid repetition.\n\n") + base;
    ...
    try {
      ...
      const answer = gpt?.choices?.[0]?.message?.content?.trim() || base;
      cache.set(cacheKey, answer);
      return res.json({ answer, took_ms: Date.now() - start });
    } catch (e) {
      cache.set(cacheKey, base);
      return res.json({ answer: base, warning: (language === "vi" ? "Không thể kết nối OpenAI: " : "OpenAI error: ") + e.message, took_ms: Date.now() - start });
    }
  } catch (err) {
...
const server = app.listen(port, async () => {
  ...
  } catch (e) { console.error("Check OpenAI key error:", e.message); }
});
