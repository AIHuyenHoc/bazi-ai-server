// server.js — Bát Tự API
// - Guard 2026–2033 (không bịa năm)
// - Chỉ nói “bổ sung Dụng Thần” (không “bồi/nuôi dưỡng hành …”)
// - Career: trả 3–5 NGHỀ CỤ THỂ khi user hỏi nghề
// - Travel/Helper/Love: dùng Dịch Mã / Thiên Ất Quý Nhân / Đào Hoa
// - “Năm nay/tháng này/tuần này”: KHÔNG chèn gợi ý năm 2026–2033
// - Văn phong lịch sự, truyền cảm, có mở–thân–kết
...
const guessLang = (messages=[]) =>
  /(bat tu|tu tru|ngay|thang|nam|gio|giap|at|binh|dinh|mau|ky|canh|tan|nham|quy|ty|suu|dan|mao|thin|ty|ngo|mui|than|dau|tuat|hoi)/i
...
const classify = (txt) => {
  ...
  const relativeNow = /(n(a|ă)m nay|th(a|á)ng n(a|ă)y|tu(a|â)n n(a|ă)y|h(ô|o)m nay|ng(a|à)y mai|this year|this month|this week|today|tomorrow)/i.test(t);
  const intents = {
  ...
  return { ...intents, hasChoice, relativeNow };
};
...
Giọng văn lịch sự, truyền cảm, có mở–thân–kết: mở đầu chào ngắn; thân bài đi thẳng trọng tâm; kết bài tóm tắt 1 câu + lời chúc tinh tế.
“Bổ sung Dụng Thần” chỉ nhắc một lần, sau đó nói bằng HÀNH ĐỘNG CỤ THỂ (thói quen/không gian/đối tác/thời điểm).
- Mộc: không gian xanh/ánh sáng tự nhiên; viết–đọc–học; vận động linh hoạt; vật liệu gỗ/giấy; hướng Đông/Đông Nam.
- Hỏa: ánh sáng/nhiệt vừa đủ; thuyết trình; cardio; bếp/điện/media; nhịp ngủ–thức đều; hướng Nam.
- Thổ: giờ giấc ổn định; kế hoạch/sổ sách; gốm–đất–làm bánh; thực dưỡng cân bằng; hướng Trung tâm/Tây Nam/Đông Bắc.
- Kim: tối giản–kỷ luật; hít thở/phổi; kim loại–công cụ–dữ liệu; luật/quy chuẩn; hướng Tây/Tây Bắc.
- Thủy: thiền thở–ngủ đủ; gần nước–di chuyển; nghiên cứu/suy tư; logistics/giao tiếp xa; hướng Bắc.
`;
const JOB_GUIDE_VI = `
Gợi ý NGÀNH/NGHỀ theo Dụng Thần:
- Mộc: giáo dục–đào tạo, ngôn ngữ/biên phiên dịch, YHCT–VLTL, thiết kế–nội dung, nông–lâm.
- Hỏa: năng lượng–điện–điện tử, ẩm thực–F&B, truyền thông–quảng cáo–MC, sự kiện–giải trí, bán hàng.
- Thổ: xây dựng–kiến trúc–QS, vận hành–PMO–QA quy trình, thực phẩm–chuỗi cung ứng, BĐS–quy hoạch, hành chính–kế hoạch.
- Kim: luật–tuân thủ–kiểm toán, tài chính–ngân hàng–chứng khoán, phân tích dữ liệu–BI, cơ khí–chế tạo–QA, an ninh–tiêu chuẩn.
- Thủy: logistics–vận tải–hàng hải, du lịch–hàng không, CNTT–hạ tầng–Cloud/SaaS, CSKH, XNK.
Khi hỏi “nên chọn công việc gì”: liệt kê 3–5 ngành CỤ THỂ, mỗi ngành 1–2 câu “vì sao hợp”.
`;
...
// context-aware year sanitizer
const sanitizeYears = (text, dungThan, { relativeNow=false } = {}) => {
  if (!text) return text;
  ...
  rest = rest.replace(/\b(20\d{2})\b/g, (m,_y,offset,s)=>{
    ...
    if (!/(năm|nam|year)\s*$/i.test(before)) return m; // giữ số năm khác context "năm ..."
    return ALLOWED_YEAR_DIGITS.has(Number(m)) ? m : "";
  });

  // Nếu câu hỏi thuộc "năm nay / tháng này / tuần này" → KHÔNG gợi ý 2026–2033
  if (relativeNow) return [head, rest].filter(Boolean).join("\n");
  ...
  if (!hasAnyAllowed && needYears) {
    ...
    const ranked = YEARS_26_33.map(y=>({
      ...
      .map(y=>`${y.year} ${y.label} (${y.elements.join(" & ")})`).join("; ");
...
const postProcess = (text, { hasChoice, yearPair, dungThan, relativeNow }) => {
  let out = (text||"").trim();

  // bỏ lời mời A/B nếu user không hỏi
  if (!hasChoice) {
  ...
  out = out.split("\n").filter(line=>{
    const m=line.match(reAge); if(!m) return true; const pair=`${cap(m[1])} ${cap(m[2])}`; return pair===yearPair;
  }).join("\n").trim();

  // sửa sai hành + guard năm
  out = fixElementMistakes(out);
  out = sanitizeYears(out, dungThan, { relativeNow });

  // loại cụm “bồi/nuôi dưỡng/tăng cường hành X”
...
app.post("/api/luan-giai-bazi", async (req, res) => {
  ...
  try {
    ...
    const nguHanhPct = percentify(nguHanh); // FE only
    ...
    if (intents.general)
      sections.push(lang==="vi"
        ? "Tổng quan: vì sao nên BỔ SUNG DỤNG THẦN (nhắc 1 lần), sau đó chuyển thành 2–3 việc làm cụ thể (thói quen/không gian/đối tác/thời điểm) theo Dụng Thần."
        : "Overview: mention SUPPLEMENTING the Useful God once, then give 2–3 concrete actions (habits/space/partners/timing) aligned with it.");

    if (intents.money)
      sections.push(lang==="vi"
        ? "Tài vận: áp dụng Dụng Thần vào ngành/nghề, đối tác, không gian làm việc, lịch làm."
        : "Wealth: apply Useful God to industries/partners/workspace/schedule.");

    if (intents.career)
      sections.push(lang==="vi"
        ? "Sự nghiệp: LIỆT KÊ 3–5 ngành/nghề CỤ THỂ hợp Dụng Thần (mỗi ngành 1–2 câu vì sao hợp)."
        : "Career: LIST 3–5 SPECIFIC professions/industries matching the Useful God (1–2 lines each).");

    // Love / Hôn nhân — dùng Đào Hoa (+ Quý Nhân nếu cần)
    if (intents.love) {
      const dao = thanSat["Đào Hoa"] || [];
      const qn = thanSat["Thiên Ất Quý Nhân"] || [];
      sections.push(
        lang==="vi"
          ? `Tình cảm/hôn nhân: mô tả phong cách giao tiếp theo Dụng Thần; nếu Đào Hoa đang kích${dao.length?` tại ${dao.join(", ")}`:""} thì gợi ý thời điểm/không gian tương ứng; có thể tận dụng Quý Nhân (${qn.join(", ")||"—"}) để kết nối.`
          : `Love/Marriage: communication style per Useful God; if Peach Blossom is active${dao.length?` at ${dao.join(", ")}`:""}, suggest matching timing/space; optionally leverage Nobleman (${qn.join(", ")||"—"}).`
      );
    }

    if (intents.health)
      sections.push(lang==="vi" ? "Sức khỏe: 1 thói quen 10–15 phút/ngày bám Dụng Thần." : "Health: one 10–15 min daily habit aligned with Useful God.");

    if (intents.family)
      sections.push(lang==="vi" ? "Gia đạo: một hành động cụ thể theo Dụng Thần để tăng hòa khí." : "Family: one concrete action per Useful God for harmony.");
    ...
    // Thời vận theo năm chỉ khi user KHÔNG hỏi “năm nay/tháng này/tuần này…”
    if (intents.timeLuck && !intents.relativeNow)
      sections.push(
        lang==="vi"
    ...
    if (intents.travel) {
      ...
      sections.push(
        lang==="vi"
          ? (dm.length
              ? `Đi lại/di chuyển: tận dụng sao Dịch Mã đang kích tại ${dm.join(", ")}; ưu tiên lịch/giờ/điểm đến gợi nhắc chi này; chuẩn bị tối giản theo Dụng Thần.`
              : `Đi lại/di chuyển: chưa có Dịch Mã kích trực tiếp; vẫn tối ưu nhờ Dụng Thần (thời điểm–không gian phù hợp, hành trình gọn–đủ).`)
          : (dm.length
              ? `Travel: leverage active Travelling Star at ${dm.join(", ")}; prefer matching dates/hours/locations; keep kit minimal per Useful God.`
              : `Travel: no direct Travelling Star; optimize via Useful God (aligned timing/space, minimal focused setup).`)
      );
    }
    ...
    if (intents.helper) {
      ...
      sections.push(
        lang==="vi"
          ? (qn.length
              ? `Người giúp đỡ: kích hoạt Thiên Ất Quý Nhân tại ${qn.join(", ")}; tìm người/dịp liên hệ các chi này, hẹn ở không gian tương ứng; mở lời lịch sự theo Dụng Thần.`
              : `Người giúp đỡ: chưa có Quý Nhân kích; ưu tiên kết nối trong ngành/hành trùng Dụng Thần và chọn thời điểm tương ứng.`)
          : (qn.length
              ? `Helpers/Nobleman: activate Tian Yi at ${qn.join(", ")}; seek people/timings linked to these branches; meet in aligned spaces; ask in a Useful-God style.`
    ...
    // system prompt — tone + rules; chỉ kèm job guide khi hỏi career
    const baseSystemVI = [
      "Bạn là trợ lý Bát Tự tinh tế. Văn phong lịch sự, truyền cảm, có mở–thân–kết rõ ràng.",
      "MỞ ĐẦU (sau câu chào ngắn): Tứ Trụ + Nhật Chủ (hành) + Dụng Thần + Thần Sát (nếu có).",
      "Chỉ dùng 'Tuổi ...' cho NĂM sinh.",
      "Dùng cụm 'BỔ SUNG DỤNG THẦN' 1 lần, sau đó chuyển thành hành động cụ thể (thói quen/không gian/đối tác/thời điểm).",
      "Không bịa mốc năm; màu sắc chỉ khi được hỏi.",
      PRACTICAL_GUIDE_VI,
    ];
    if (intents.career) {
      baseSystemVI.push(
        "Khi hỏi 'nên chọn công việc gì/career/job': BẮT BUỘC liệt kê 3–5 ngành/nghề CỤ THỂ hợp Dụng Thần, mỗi ngành 1–2 câu vì sao hợp.",
        JOB_GUIDE_VI
      );
    }
    ...
    const baseSystemEN = [
      ...
      "Open with a brief greeting, then: Four Pillars + Day Master(element) + Useful God + Active Stars.",
      "Say 'SUPPLEMENT THE USEFUL GOD' once; then give concrete actions (habits/space/partners/timing).",
      "Courteous tone with clear beginning–body–closing. No invented years; color only if asked.",
    ];
    if (intents.career) {
      baseSystemEN.push("When asked about career/jobs, list 3–5 SPECIFIC professions/industries aligned with the Useful God (1–2 lines each).");
    }
    ...
    if (intents.travel) {
      const dm = thanSat["Dịch Mã"] || [];
      contextParts.push(lang==="vi" ? `Hint đi lại: Dịch Mã = ${dm.join(", ")||"—"}.` : `Travel hint: Travelling Star = ${dm.join(", ")||"—"}.`);
    }
    if (intents.helper) {
      const qn = thanSat["Thiên Ất Quý Nhân"] || [];
      contextParts.push(lang==="vi" ? `Hint Quý Nhân: ${qn.join(", ")||"—"}.` : `Nobleman hint: ${qn.join(", ")||"—"}.`);
    }
    if (sections.length) contextParts.push(lang==="vi" ? `Trọng tâm:\n- ${sections.join("\n- ")}` : `Focus:\n- ${sections.join("\n- ")}`);
    ...
    // cache key theo intent
    const intentKey = Object.entries(intents).filter(([_,v])=>v).map(([k])=>k).sort().join(",");
    ...
    try {
      ...
      // fallback: mở–thân–kết ngắn gọn
      answer = (lang==="vi" ? "Chào bạn,\n\n" : "Hi,\n\n") +
        core +
        (lang==="vi"
          ? `\n\nTóm lại, hãy bắt đầu **bổ sung Dụng Thần** (${dungThanHanh.join(", ")}), biến thành 2–3 thói quen/không gian cụ thể mỗi ngày. Chúc bạn hanh thông!`
          : `\n\nIn short, **supplement your Useful God** (${dungThanHanh.join(", ")}), turn it into 2–3 concrete habits/space tweaks. Wishing you clarity and flow!`);
      try { fs.appendFileSync("error.log", `${new Date().toISOString()} | /api/luan-giai-bazi | OpenAI error: ${e.stack||e.message}\n`);} catch {}
    }

    answer = postProcess(answer, { hasChoice:intents.hasChoice, yearPair:tuTru.nam, dungThan:dungThanHanh, relativeNow:intents.relativeNow });
