...
/* ======== Bát Tự API (GPT-3.5 viết lời; nội bộ chỉ tính dữ kiện) ======== */
...
const guessLang = (messages = []) => {
  ...
  return /ngay|thang|nam|gio|giap|at|binh|dinh|mau|ky|canh|tan|nham|quy|ty|suu|dan|mao|thin|ty|ngo|mui|than|dau|tuat|hoi/i.test(
    rm(t)
  )
    ? "vi"
    : "en";
...
const tinhThanSat = (tuTru) => {
  ...
  const daoHoaMap = { "Thân": "Dậu", "Tý": "Dậu", "Thìn": "Dậu", "Tỵ": "Ngọ", "Dậu": "Ngọ", "Sửu": "Ngọ", "Dần": "Mão", "Ngọ": "Mão", "Tuất": "Mão", "Hợi": "Tý", "Mão": "Tý", "Mùi": "Tý" };
  ...
  return {
    ...
    "Đào Hoa": daoHoaMap[ngayChi] && branches.includes(daoHoaMap[ngayChi]) ? [daoHoaMap[ngayChi]] : [],
  };
};
...
app.post("/api/luan-giai-bazi", async (req, res) => {
  ...
  try {
    ...
    const fp = JSON.stringify({
      ...
      dungs,
      intents,
      choice: choice ? [choice.a, choice.b] : null,
      q: rm(userInput).slice(0, 160),
    });
    const cached = cache.get(fp);
    if (cached) return res.json({ answer: cached, cached: true });

    // 3 câu trước của assistant để hạn chế lặp
    const prevAssistant = messages.filter((m) => m.role === "assistant").slice(-3).map((m) => m.content);

    const intentRubricVI = {
      general:
        "Mở đầu 1 câu riêng (không dùng 'Dựa vào bát tự'). Nêu điểm mạnh/yếu ngũ hành (không liệt kê %). Gắn Dụng Thần nếu có. Đưa 2–3 gợi ý ứng dụng hàng ngày.",
      money:
        "Trọng tâm tài chính: cách kiểm soát dòng tiền/tiết kiệm/đầu tư hợp hành của Dụng Thần; 2 bước hành động; 1 rủi ro cần tránh.",
      career:
        "Trọng tâm sự nghiệp: 3 hướng nghề/nhánh công việc phù hợp với hành của Dụng Thần + 1 kỹ năng nên rèn trong 7 ngày.",
      love:
        "Trọng tâm tình cảm: kiểu người hợp (ngũ hành/chi), nếu có lựa chọn A/B hãy chọn 1 bên và ghi 2 lý do (hợp chi, hợp hành, Đào Hoa).",
      health:
        "Trọng tâm sức khỏe: giấc ngủ, nhịp năng lượng của hành yếu; 2 thói quen nhỏ (10–15 phút) để cân bằng.",
      family:
        "Trọng tâm gia đạo: cách giao tiếp/hỗ trợ thế hệ trong nhà theo hành Dụng Thần; 1 việc nên làm cuối tuần.",
      luck:
        "Trọng tâm thời điểm: khung giờ/không gian hợp hành (không nêu năm/tháng cụ thể); 1 nghi thức nhỏ để khởi tâm.",
      color:
        "Trọng tâm màu sắc & phong cách: 1–2 tông màu hợp hành + cách phối ứng dụng vào công việc/hẹn hò.",
    };
    ...
    const bannedPhrasesVI = [
      ...
      "Với bát tự của bạn",
      "Theo bát tự",
      "Ngũ hành Kim xuất hiện nhiều, tượng trưng cho",
      "Thần Sát đang hoạt động",
      "hãy tránh việc làm quá mức",
      "Hãy dành thời gian để chăm sóc",
    ];

    const system =
      language === "vi"
        ? "Bạn là Thầy Bát Tự AI. Viết tự nhiên, truyền cảm, KHÔNG dùng cụm sáo rỗng; mở đầu đa dạng. Trả lời đúng trọng tâm câu hỏi hiện tại; chỉ nhắc màu khi người dùng hỏi. Liên hệ Dụng Thần/Thần Sát cụ thể (gọi tên sao nếu có). Không bịa mốc năm/tháng. Độ dài 120–180 từ. Dùng vài gạch đầu dòng nếu giúp dễ đọc."
