...
app.post("/api/luan-giai-bazi", async (req, res) => {
  ...
  try {
    ...
    if (intents.money)
      sections.push(
        lang === "vi"
          ? "Tài vận: gắn Dụng Thần vào cách kiếm/giữ tiền; tận dụng Quý Nhân/Dịch Mã/Đào Hoa nếu có; không nói chung chung."
          : "Wealth: tie Useful God to ways of earning/saving; leverage Nobleman/Travel/Peach Blossom if active."
      );
    if (intents.career)
      sections.push(
        lang === "vi"
          ? "Sự nghiệp: gợi ý ngành/kiểu làm việc hợp Dụng Thần; tránh lan man."
          : "Career: roles/process aligned with Useful God; keep focused."
      );
    if (intents.love)
      sections.push(
        lang === "vi"
          ? "Tình cảm: dùng Đào Hoa/Quý Nhân (nếu có) + Dụng Thần để gợi ý phong cách giao tiếp/hẹn hò."
          : "Love: use Peach Blossom/Nobleman + Useful God for communication/dating style."
      );
    if (intents.health)
      sections.push(
        lang === "vi"
    ...
    if (intents.family)
      sections.push(
        lang === "vi"
          ? "Gia đạo: một hành động cụ thể để tăng hòa khí dựa trên Dụng Thần."
          : "Family: one concrete action for harmony based on Useful God."
      );
    if (intents.children)
      sections.push(
        lang === "vi"
          ? "Con cái: định hướng học tập/nuôi dạy khớp Dụng Thần."
          : "Children: study/parenting orientation aligned with Useful God."
      );
    if (intents.color)
      sections.push(
        lang === "vi"
          ? "Màu sắc/phong cách: chỉ trả lời khi được hỏi; quy palette theo Dụng Thần."
          : "Color/style: only when asked; map palette to Useful God."
      );
    if (intents.timeLuck)
      sections.push(
        lang === "vi"
